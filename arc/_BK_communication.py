...
# Serial setting
ser = serial.Serial('/dev/ttyUSB0', 115200)  # Change port number here
time.sleep(2)
...
def get_model():
    model = torch.hub.load("../yolov5-new-main", 'custom', path="/home/kamata/Program/yolov5-new-main/models/BKweights_epoch_150.pt", source='local')
    return model
...
def yolo_detection(model, camera_index=0, exposure_value=750):
    model.conf = 0.55
    objects = [0]
    ...
    objects_sorted = objects.sort_values(by='xmin')
